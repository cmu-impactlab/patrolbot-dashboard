"""Charging / motor-power / undock behavior of the simulated robot.

The mock is what the dashboard is demonstrated and developed against, so it
claims the capabilities the commissioned robot claims — notably no `dock`,
because the robot has no automatic dock-in path. The real bridge advertises
each one only while its dock-manager server is actually up; the mock, having no
such server to lose, simply claims all three. Undock is a single operator action: the
robot opens its own charger and powers its own motors as part of executing it,
and reverses in a straight line rather than turning on the dock.
...
def test_capabilities_match_the_real_robot():
    """The mock claims what the real robot claims. `dock` is absent from both:
    the robot has no automatic dock-in path, and a mock that pretended
    otherwise is what let the missing path go unnoticed."""
    from mock_robot.client import CAPABILITIES

    for command in ("charge_release", "motor_enable", "undock"):
        assert command in CAPABILITIES
    assert "dock" not in CAPABILITIES

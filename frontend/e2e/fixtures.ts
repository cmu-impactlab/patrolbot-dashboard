...
export async function dashboardFixture(page: Page, role = "operator", preset = "Operator", largeMap = false) {
  ...
  const send = (type: string, data: unknown) => socket.send(JSON.stringify({ version: 1, type, robot_id: "mock", sequence: ++sequence, timestamp: new Date().toISOString(), data }));
  ...
  await page.routeWebSocket("**/ws/ui", ws => {
    ...
    send("server.snapshot", { connection: { state: "online" }, robot_status: { status: "ready", detail: "Ready" },
      ...
      base_state: { session_generation: 1, link_connected: true, telemetry_age: 0.1, hardware_state_valid: true, charge_state: "not_charging", motors_enabled: true, estop_pressed: false, fault_flags: 0, stall_value: 0, bumpers_front: false, bumpers_rear: false, bumpers_valid: true },
  ...
  return { commands, saves, send, setFailSave: (value: boolean) => { failSave = value; }, setCurrent: (doc: LayoutDoc) => { current = doc; } };
}
